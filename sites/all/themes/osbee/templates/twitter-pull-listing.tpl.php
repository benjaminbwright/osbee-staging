  ...
  <?php if (is_array($tweets)): ?>
    ...
    <?php foreach ($tweets as $tweet_key => $tweet): ?>
      <li>
        ...
        <span class="tweet-author"><?php print l($tweet->username, 'http://twitter.com/' . $tweet->screenname, array('attributes' => array('target'=>'_blank'))); ?></span>
        <span class="tweet-text"><?php print twitter_pull_add_links($tweet->text); ?></span>
        <div class="tweet-time"><?php print l($tweet->time_ago, 'http://twitter.com/' . $tweet->screenname . '/status/' . $tweet->id, array('attributes' => array('target'=>'_blank')));?></div>
